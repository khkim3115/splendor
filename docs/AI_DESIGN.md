...
export function mctsChoose(view: GameState, me: number, budgetMs: number, rng: RngState): [Action, RngState] {
  ...
  while (true) {
    ...
    // (M6-2 L1 프루닝 적용으로 6으로 축소 — src/ai/mcts.ts PLAYOUT_DEPTH, §4.4 사다리)
...
**M6 판정 기록 (DoD 1, 확정)**: L1 적용 후에도 실측 ~380 iter/s로 수용 하한(800)에는 미달. 그러나 사다리의 목적 게이트인 어려움>보통 승률 밴드(65~80%)가 충족되어(§6.1 M6 기록) **L2(알고리즘 교체)는 미적용**. ROADMAP M6 DoD의 "미달 시 L1/L2 적용 후 재측정 기록 존재"는 L1 적용 + 재측정 기록(`tests/bench/bench.test.ts`, §6.1)으로 충족 판정.
...
- 도구: `tests/ai/arena.selfplay.test.ts` (`SELFPLAY=1` 가드, Node, 브라우저 불필요 — 엔진·AI가 순수 모듈이라 vitest에서 그대로 구동. 당초 계획의 `scripts/selfplay.ts`를 아레나 테스트로 대체 — 이슈 #3).
...
- **M6 측정 기록 (2026-07, `tests/ai/arena.selfplay.test.ts`, 어려움 예산 500ms — `ARENA_BUDGET_MS`)**: 어려움 > 보통 **72.9%** (145/199 결정판, 무승부 1, 200판, 시드 11000+g, **freeze OFF — 프로덕션 동등 조건**) — **밴드(65~80%) 내, M6 DoD 충족**. 보통 > 쉬움 **88.0%** (176/200, 시드 9000+g — M5와 동일 시드로 동일 수치 재현, 아레나의 chooseAction 라우팅 전환 등가성 검증 겸함). 어려움 > 쉬움 **92.0%** (46/50, 시드 13000+g, freeze ON — 아래 측정 조건 주석). **서열 어려움>보통>쉬움 명확.** 3~4인 혼합 스모크(3인 hard/normal/easy 25판 + 4인 hard/normal/easy/easy 25판, 좌석 로테이션, 시드 15000+g, freeze ON) 50판 크래시·교착 없이 gameOver 종료 — 승수(공동승 포함) hard 29 / normal 19 / easy 2로 다인전에서도 서열 유지.
  - **측정 조건 — deep-freeze (최종 리뷰 반영)**: 엔진의 dev 전용 deep-freeze 가드는 기본 ON이지만 프로덕션 Worker(src/ai/worker.ts)·벤치는 OFF로 돈다. 최초 M6 인증 측정은 아레나가 freeze ON인 채 수행됐음이 발견됐다(어려움>보통 **69.8%** — freeze가 apply 단가를 30~70% 올려 어려움의 예산 내 유효 iteration을 깎는 조건). 아레나에 `setStateFreezing(false)`를 넣고 같은 시드·예산으로 재측정한 **72.9%가 인증 수치**다(파일럿 50판 74.0% — freeze OFF로 어려움 유효 iteration이 늘어 상향, 예상 방향). 보통>쉬움은 재측정 불요 — 쉬움/보통은 고정 깊이 탐색이라 freeze가 벽시계만 바꾸고 착수는 불변. 어려움>쉬움(92.0%)·혼합 스모크는 freeze ON 측정치 유지 — freeze OFF는 어려움을 강하게만 하므로 서열 확인(≥65%)·정상 종료 판정은 보수적으로 유효하다.
  - **예산 민감도** (어려움>보통 50판, 시드 11000+g, freeze ON 측정): 150ms **58%** / 400ms **68%** / 1,000ms **90%**. "150ms로도 서열 검증 충분"은 2,000 sim/s 가정이었고 실측은 ~380 iter/s — 150ms는 ~57 iteration이라 MCTS가 강도를 내기 전이다. **아레나 기본 예산을 500ms(출하 예산의 1/2, ~190 iteration)로 상향**해 측정·기록한다. (사다리의 용도는 예산 선정의 방향 판단 — freeze OFF에서도 예산 단조 증가 구조는 동일하다.)
  - **파라미터 판정**: UCB_C 하향 탐색(§4.1) 결과 0.2→0.1이 400ms 50판(freeze ON A/B)에서 68%→60%로 악화(저반복 구간에서는 탐험 축소가 루트 선별 1위 쏠림 = 1-ply 퇴화를 유발) — **0.2 유지**. WEIGHTS·softmax(topK/온도)도 무변경 — 보통>쉬움 88.0%의 밴드 소폭 초과는 M5 결정대로 수용(온도 비지배 + simple 상향은 콘셉트 상충), 서열이 명확하므로 3단계 균형을 깨지 않는 것이 우선.
  - **풀 예산 참고**: 출하 구성(1,000ms, freeze ON 측정)에서 어려움>보통 90%(50판)로 밴드 상회. 어려움을 의도적으로 약화하는 것은 "어려움은 실제로 잘 둬야 함"(M6 목표)과 상충하고, 보통 상향은 보통>쉬움 초과를 키운다 — 인접 밴드 관리는 아레나 예산(500ms) 기준으로 하고 최상위 난이도의 실전 강함은 설계 의도로 수용한다.
- 3~4인전 스모크(혼합 난이도 50판)로 다인전 붕괴 여부만 확인.
- **CI 정책** (심사 지적 4 반영): PR 게이트 아님. `ai-arena.yml` **주 1회**, 어려움 예산 150ms로 축소 실행(절대 강도는 낮아지지만 서열 검증에는 충분). (M6-3 실측 후 정정: 150ms에서는 어려움>보통 58%로 서열 검증이 안 된다 — CI를 만들 경우 500ms 기준 권장, 위 M6 기록 참조.) 추정 실행시간 60~70분 — Actions 한도 내. 정밀 측정(1,000ms 예산)은 로컬 실행이 기본.
