...
- **M6 측정 기록 (2026-07, `tests/ai/arena.selfplay.test.ts`, 어려움 예산 500ms — `ARENA_BUDGET_MS`)**: 어려움 > 보통 **69.8%** (139/199 결정판, 무승부 1, 200판, 시드 11000+g) — **밴드(65~80%) 내, M6 DoD 충족**. 보통 > 쉬움 재측정 **88.0%** (176/200, 시드 9000+g — M5와 동일 시드로 동일 수치 재현, 아레나의 chooseAction 라우팅 전환 등가성 검증 겸함). 어려움 > 쉬움 **92.0%** (46/50, 시드 13000+g). **서열 어려움>보통>쉬움 명확.** 3~4인 혼합 스모크(3인 hard/normal/easy 25판 + 4인 hard/normal/easy/easy 25판, 좌석 로테이션, 시드 15000+g) 50판 크래시·교착 없이 gameOver 종료 — 승수(공동승 포함) hard 29 / normal 19 / easy 2로 다인전에서도 서열 유지.
  - **예산 민감도** (어려움>보통 50판, 시드 11000+g): 150ms **58%** / 400ms **68%** / 1,000ms **90%**. "150ms로도 서열 검증 충분"은 2,000 sim/s 가정이었고 실측은 ~380 iter/s — 150ms는 ~57 iteration이라 MCTS가 강도를 내기 전이다. **아레나 기본 예산을 500ms(출하 예산의 1/2, ~190 iteration)로 상향**해 측정·기록한다.
  - **파라미터 판정**: UCB_C 하향 탐색(§4.1) 결과 0.2→0.1이 400ms 50판에서 68%→60%로 악화(저반복 구간에서는 탐험 축소가 루트 선별 1위 쏠림 = 1-ply 퇴화를 유발) — **0.2 유지**. WEIGHTS·softmax(topK/온도)도 무변경 — 보통>쉬움 88.0%의 밴드 소폭 초과는 M5 결정대로 수용(온도 비지배 + simple 상향은 콘셉트 상충), 서열이 명확하므로 3단계 균형을 깨지 않는 것이 우선.
  - **풀 예산 참고**: 출하 구성(1,000ms)에서 어려움>보통 90%(50판)로 밴드 상회. 어려움을 의도적으로 약화하는 것은 "어려움은 실제로 잘 둬야 함"(M6 목표)과 상충하고, 보통 상향은 보통>쉬움 초과를 키운다 — 인접 밴드 관리는 아레나 예산(500ms) 기준으로 하고 최상위 난이도의 실전 강함은 설계 의도로 수용한다.
