...
## 데스크톱 트레이 앱 (Windows · macOS)

작업 중에도 화면 구석에 작게 띄워두는 **은밀한 트레이 상주 앱**입니다. 극소형·무채색으로 보드/상대/귀족을 접었다 펴며 훑어보고, 전역 보스키(`Ctrl+Alt+Space`)로 즉시 숨기고 부를 수 있습니다.

**▶ 다운로드: [최신 릴리스](https://github.com/khkim3115/splendor/releases/latest)**

- **Windows**: `Splendor-Tray-Setup.exe` 실행 → 설치. 이후 자동 업데이트를 지원합니다.
- **macOS**: `Splendor-Tray.dmg` 열어 앱을 Applications로 이동. 코드서명이 없어(ad-hoc) 첫 실행 시 **우클릭 → 열기**로 Gatekeeper를 통과시켜야 합니다.

조작: 상단 바를 잡아 창 이동, `🔅` 투명도 조절, `☀️/🌙` 테마 전환, `Esc` 또는 `✕`로 숨기기. 게임 화면에선 `B/O/N`(펼침) · `L`(글자코드 언어) · `1~5`(토큰 집기) · `Enter`(확정) · `U`(무르기) · `P`(패스) 단축키를 지원합니다.
